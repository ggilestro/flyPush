...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'approved', 'rejected', name='orgjoinrequeststatus'), default='pending'),
    ...
    op.create_index('ix_org_join_requests_org_id', 'organization_join_requests', ['organization_id'])
    ...
    op.create_table(
        ...
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('tray_type', sa.Enum('numeric', 'grid', 'custom', name='traytype'), default='numeric'),
    ...
    op.create_table(
        ...
        sa.Column('origin', sa.Enum('repository', 'internal', 'external', name='stockorigin'),
                  default='internal', server_default='internal'),
        sa.Column('repository', sa.Enum('bdsc', 'vdrc', 'kyoto', 'nig', 'dgrc', 'flyorf', 'trip', 'exelixis', 'other',
                                        name='stockrepository'), nullable=True),
        ...
        sa.Column('visibility', sa.Enum('lab_only', 'organization', 'public', name='stockvisibility'),
                  default='lab_only', server_default='lab_only'),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('planned', 'in_progress', 'completed', 'failed', name='crossstatus'), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'approved', 'rejected', 'fulfilled', 'cancelled',
                                    name='stockrequeststatus'), default='pending'),
        sa.Column('message', sa.Text(), nullable=True),
    ...
    op.create_index('ix_stock_requests_requester_tenant_id', 'stock_requests', ['requester_tenant_id'])
